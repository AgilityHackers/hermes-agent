...
export const zh: Translations = {
  ...
  assistant: {
    ...
    mcpSetup: {
      ...
      dismiss: '关闭',
      declined: '已拒绝',
      declinedOne: server => `已拒绝 ${server}`,
      connected: server => `已连接 ${server}`,
      skipped: server => `已跳过 ${server}`,
      failed: server => `无法连接 ${server}`,
      unanswered: '未响应',
      toolCount: count => `${count} 个工具`,
      notFound: connectors => `找不到 ${connectors}`,
      catalogSource: '来自 Nous 认证目录',
      stateDisabled: '当前已关闭',
      stateNeedsAuth: '未登录',
      trustCatalog: '已审核',
      trustOfficial: '官方',
      trustVerified: publisher => `已验证 · ${publisher}`,
      trustVerifiedTip: publisher => `发布者已证明拥有该端点所在域名 ${publisher}。这是身份验证，并不代表推荐。`,
      trustCommunity: '未审核',
      trustCommunityTip: '无人验证该端点的运营方 — 发布者的注册名称与其连接的域名不匹配。连接前请先检查地址。',
      envRequired: '请先填写所需凭据',
      sendFailed: '无法发送连接器设置响应',
      reloadFailed: '连接器已保存，但重新加载其工具失败 — 将在下个会话加载',
      gatewayDisconnected: 'Hermes 网关未连接'
    },
    tool: {
      copyCode: '复制代码',
